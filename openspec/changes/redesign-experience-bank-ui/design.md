...
- `ExperienceAchievementRow.svelte` — one achievement: display / editing / promote-to-project
  modes, unchanged behavior, restyled. Takes the atom and callbacks as props; selection state
  (`selected`) stays lifted in the parent so it survives any card's expand/collapse.
...
first-open behavior). Rejected alternative: lift `expanded` for every employment into the
parent's own state map — more central state to keep in sync for no behavioral benefit, since
nothing outside one card ever needs to know another card's expand state except this one
banner-jump case, which only needs to *set* it.
...
was already `border-transparent`). The metrics tags (`bg-muted ... font-mono`) and the
`cluster_id`/`needs_context`/`needs_metrics` flags (`border border-border`) are left as
`Chip variant="default"` and a borderless muted style respectively — they are not the skill
vocabulary and don't need an icon slot, so `Chip` without a variant already covers them without
