...
- [ ] 1.1 Write a failing vitest test for a `findFirstUnconfirmed(bank)` helper: given an
...
- [ ] 1.2 Implement `findFirstUnconfirmed` (e.g. `web/src/lib/experienceBank.ts`) to pass 1.1.

## 2. Extract `ExperienceAchievementRow.svelte`

- [ ] 2.1 Move the `achievement` snippet out of `ExperienceBankView.svelte` into
...
- [ ] 2.2 Remove the row's border; keep background only for the selected/unconfirmed states, at
      reduced visual weight per design.md.
- [ ] 2.3 Replace the hand-rolled skill-tag `<span>` markup with `<Chip variant="brand">`, and
...
- [ ] 2.4 Rework the achievement edit form (claim / context / metrics) onto `FormField` +
      `Input`/`textarea`, keeping the same fields and the "Save — this makes it yours" copy.
- [ ] 2.5 Add a `scrollToAtomId` prop: when it equals this row's atom id, scroll the row into
      view and move focus to it.

## 3. Extract `ExperienceEmploymentCard.svelte`

- [ ] 3.1 Move the `employmentSection` snippet into
      ...
      each atom.
- [ ] 3.2 Add local `expanded = $state(false)`; render a collapsed summary (achievement count,
      or "nothing recorded yet") by default, and the achievement list only when expanded.
- [ ] 3.3 Accept `forceExpanded?: boolean` and `scrollToAtomId?: string` props; expand the card
      when `forceExpanded` is true and forward `scrollToAtomId` to the matching row.
- [ ] 3.4 Replace the stack `<span>` tag markup with `<Chip variant="brand">`.
- [ ] 3.5 Rework the employment edit-in-place form onto `FormField` + `Input`/`textarea`.
- [ ] 3.6 Remove the card's border.

## 4. Wire the parent view

- [ ] 4.1 Replace the inline `employmentSection`/`achievement` snippet usages in
...
- [ ] 4.2 Add `bannerTarget = $state<{ employmentId: string | null; atomId: string } | null>`;
      make the "N not confirmed" banner an activatable control (button/link) that sets it via
      `findFirstUnconfirmed(bank)`.
- [ ] 4.3 Pass `forceExpanded`/`scrollToAtomId` down to the matching `ExperienceEmploymentCard`
      (by `employmentId`) or to the unplaced section's rows (when `employmentId` is `null`),
      derived from `bannerTarget`.
- [ ] 4.4 Rework the "Add experience" / "Add project" forms onto `FormField` + `Input`.

## 5. Visual hierarchy pass

- [ ] 5.1 Review title/company/date/meta typography and spacing across the two new components
      against design.md's "quieter secondary text, more breathing room" goal; no information
      added or removed, styling only.
- [ ] 5.2 Confirm the unconfirmed/selected background treatments still read clearly without the
      borders they used to sit inside.

## 6. Verify

- [ ] 6.1 `pnpm --filter web test` (vitest) — the new helper test from task 1 passes.
- [ ] 6.2 `pnpm --filter web check` and `pnpm --filter web lint` on the changed files.
- [ ] 6.3 `pnpm --filter web dev` and manually verify every scenario in
      ...
      render as `Chip`.
- [ ] 6.4 Remove now-dead code left in `ExperienceBankView.svelte` after the extraction (unused
